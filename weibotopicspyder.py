...
# 插入数据
def insert_data(elems, path, name, yuedu, taolun, keyword):
    for elem in elems:
        ...
        rid = rows_old
        # 用户名
        weibo_username = elem.find_elements(by=By.CSS_SELECTOR, value='h3.m-text-cut')[0].text
        ...
        try:
            ...
            weibo_userlevel = "普通用户"
        # 微博内容
        weibo_content = elem.find_elements(by=By.CSS_SELECTOR, value='div.weibo-text')[0].text
        shares = elem.find_elements(by=By.CSS_SELECTOR, value='i.m-font.m-font-forward + h4')[0].text
        comments = elem.find_elements(by=By.CSS_SELECTOR, value='i.m-font.m-font-comment + h4')[0].text
        likes = elem.find_elements(by=By.CSS_SELECTOR, value='i.m-icon.m-icon-like + h4')[0].text
        # 发布时间
        weibo_time = elem.find_elements(by=By.CSS_SELECTOR, value='span.time')[0].text

        # 某些微博内容中还含有话题，把话题去掉，如去掉#国庆阅兵#
...
def get_current_weibo_data(elems, book_name_xls, name, yuedu, taolun, maxWeibo):
    ...
    while True:
        ...
        time.sleep(3)
        elems = driver.find_elements(by=By.CSS_SELECTOR, value='div.card.m-panel.card9')
        ...
        if n == 5:
            ...
            insert_data(elems, book_name_xls, name, yuedu, taolun, keyword)
            break
...
def spider(driver, book_name_xls, sheet_name_xls, keyword, maxWeibo):
    ...
    while 1:  # 循环条件为1必定成立
        ...
        if result == 1:
            elems = driver.find_elements(by=By.CSS_SELECTOR, value='div.line-around.layout-box.mod-pagination > a:nth-child(2) > div > select > option')
    ...
    while 1:  # 循环条件为1必定成立
        ...
        elems = driver.find_elements(by=By.CSS_SELECTOR,
                                     value='div.line-around.layout-box.mod-pagination > a:nth-child(2) > div > select > option')
        # return elems #如果封装函数，返回页面
        break
    ...
    # 搜索关键词
    elem = driver.find_element(by=By.XPATH, value="//*[@class='m-text-cut']").click()
    time.sleep(2)
    elem = driver.find_element(by=By.XPATH, value="//*[@type='search']")
...
if __name__ == '__main__':
    ...
    book_name_xls = "知网.xls"  # 填写你想存放excel的路径，没有文件会自动创建
    sheet_name_xls = '微博数据'  # sheet表名
    maxWeibo = 200  # 设置最多多少条微博
    keywords = ["#知网#", ]  # 此处可以设置多个超话关键词
